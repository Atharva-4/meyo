...
public:
	...
	vertex() : 
		...
		_index(-1), _QTriArea(0), _texU(0.0f), _texV(0.0f)
	{
		initQuadric();
	};
...
//	operator>>(istream& is, vertex& vi);
	...
	{
		// Since a vertex typically only has about 6 neighbors, a linear search
		// in a vector is extremely fast and avoids the heavy memory overhead of a set.
		for (size_t i = 0; i < _vertNeighbors.size(); ++i) {
			if (_vertNeighbors[i] == v) return;
		}
		_vertNeighbors.push_back(v);
	}
	...
	{
		for (size_t i = 0; i < _vertNeighbors.size(); ++i) {
			if (_vertNeighbors[i] == v) {
				_vertNeighbors.erase(_vertNeighbors.begin() + i);
				return 1;
			}
		}
		return 0;
	...
	{
		// Triangles are intentionally allowed to be duplicates if added multiple times
		// but typically addTriNeighbor is only called once per triangle per vertex.
		for (size_t i = 0; i < _triNeighbors.size(); ++i) {
			if (_triNeighbors[i] == t) return;
		}
		_triNeighbors.push_back(t);
	}
	...
	{
		for (size_t i = 0; i < _triNeighbors.size(); ++i) {
			if (_triNeighbors[i] == t) {
				_triNeighbors.erase(_triNeighbors.begin() + i);
				return 1;
			}
		}
		return 0;
	...
	const vector<int>& getVertNeighbors() const {return _vertNeighbors;}
	vector<int>& getVertNeighbors() {return _vertNeighbors;}
	const vector<int>& getTriNeighbors() const {return _triNeighbors;}
	vector<int>& getTriNeighbors() {return _triNeighbors;}

	bool hasVertNeighbor(int v) const {
		for (size_t i = 0; i < _vertNeighbors.size(); ++i) if (_vertNeighbors[i] == v) return true;
		return false;
	}
	bool hasTriNeighbor(int t) const {
		for (size_t i = 0; i < _triNeighbors.size(); ++i) if (_triNeighbors[i] == t) return true;
		return false;
	}
	...
	void setTexCoords(float u, float v) { _texU = u; _texV = v; }
	float getU() const { return _texU; }
	float getV() const { return _texV; }
...
private:
	...
	vector<int> _vertNeighbors; // connected to this vertex via an edge
	vector<int> _triNeighbors; // triangles of which this vertex is a part
	...
	float _texU, _texV; // Texture coordinates
