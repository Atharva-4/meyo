...
{
	vector<int>& triNeighbors = vc.getTriNeighbors();
	...
	for (pos = triNeighbors.begin(); pos != triNeighbors.end(); ++pos) 
	{
		...
		{
			ec._trisRemoved.push_back(triIndex);
		...
		else
		{
			...
			ec._trisAffected.push_back(triIndex);
		}
									...
									set<int> &affectedQuadricVerts)
{
	...
	vector<int>& mytriNeighbors = vert.getTriNeighbors();
...
void PMesh::buildEdgeCollapseList(Mesh &mesh, const EdgeCost &cost, 
								  list<EdgeCollapse> &edgeCollList,
									...
									vector<vertexPtrSet::iterator> &vertSetVec)
{
	int currentTriCount = mesh.getNumTriangles();
	int count = 0;
	int totalVerts = (int)vertSet.size();

	for (;;)
	{
		if (count % 5000 == 0 && count > 0) {
			std::cout << "." << std::flush;
			if (count % 500000 == 0) std::cout << " [" << count << " / " << totalVerts << "]\n";
		}
		count++;
		...
		currentTriCount -= (int)ec._trisRemoved.size();
...
void PMesh::createEdgeCollapseList()
{
	...
	std::cout << "Step 1/3: Calculating Quadric matrices..." << std::endl;
	...
	std::cout << "Step 2/3: Calculating initial edge collapse costs..." << std::endl;
	...
	std::cout << "Step 3/3: Building edge collapse list (this takes the most time)..." << std::endl;
...
double PMesh::shortEdgeCollapseCost(Mesh& m, vertex& v)
{
	...
	vector<int>& neighbors = v.getVertNeighbors();
	vector<int>::iterator pos;
...
void PMesh::calcMelaxMaxValue(Mesh &mesh, set<int> &adjfaces, 
							  vertex &v, const vector<int> &tneighbors,
								float &retmaxValue, 
								bool &bMaxValueFound)
{
...
{
	vector<int>& vneighbors = v.getVertNeighbors();
	vector<int>& tneighbors = v.getTriNeighbors();
	vector<int>::iterator pos;
	vector<int>::iterator pos2;
	...
	for (pos = vneighbors.begin(); pos != vneighbors.end(); ++pos) 
	{
		...
		if (bMaxValueFound)
		{
			...
			if (checkCollapseFlip(mesh, v.getIndex(), *pos)) 
			{
				retmaxValue *= 1000.0f; // penalize flips
			}
...
// Check if collapsing vFrom to vTo flips any neighbor triangle normal
bool PMesh::checkCollapseFlip(Mesh& m, int vFrom, int vTo)
{
	vertex& vf = m.getVertex(vFrom);
	Vec3 fromPos = vf.getXYZ();
	Vec3 toPos = m.getVertex(vTo).getXYZ();
	
	vector<int>& tris = vf.getTriNeighbors();
	vector<int>::iterator it;
	for (it = tris.begin(); it != tris.end(); ++it)
	{
		triangle& t = m.getTri(*it);
		if (!t.isActive()) continue;
		if (t.hasVertex(vTo)) continue; // skip triangles that will be removed
		
		int v[3];
		t.getVerts(v[0], v[1], v[2]);
		
		// Current normal
		Vec3 nOrig = t.getNormalVec3();
		
		// Simulated normal after collapse
		Vec3 p[3];
		for (int i=0; i<3; i++) {
			if (v[i] == vFrom) p[i] = toPos;
			else p[i] = m.getVertex(v[i]).getXYZ();
		}
		
		Vec3 v1 = p[1] - p[0];
		Vec3 v2 = p[2] - p[1];
		Vec3 nNew = v1.unitcross(v2);
		
		// If normal flips (dot product < 0), return true
		if (nOrig.dot(nNew) < 0.1f) return true; // check for alignment or extreme bend
	}
	return false;
}
...
double PMesh::quadricCollapseCost(Mesh& m, vertex& v)
{
	...
	for (pos = neighbors.begin(); pos != neighbors.end(); ++pos) 
	{
		...
		// Penalty for flips
		if (checkCollapseFlip(m, v.getIndex(), *pos))
		{
			cost = (cost + 1e-6) * 1000.0;
		}
...
bool PMesh::splitVertex()
{
	...
	_nVisTriangles += (int)ec._trisRemoved.size();
