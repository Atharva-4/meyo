...
struct EdgeCollapse
{
	int _vfrom;
	int _vto;
	vector<int> _trisRemoved;
	vector<int> _trisAffected;
...
public:
	...
	int getCurrentCollapseCount() { return _currentCollapseCount; }

	// Set target number of collapses
	void setCollapseCount(int target) {
		if (target < 0) target = 0;
		if (target > (int)_edgeCollList.size()) target = (int)_edgeCollList.size();
		
		while (_currentCollapseCount < target) {
			if (!collapseEdge()) break;
		}
		while (_currentCollapseCount > target) {
			if (!splitVertex()) break;
		}
	}
	...
	bool saveSimplifiedMesh(char* filename) {
		return _newmesh.saveToFile(filename);
	}
	Mesh* getMesh() { return &_newmesh; }
...
private:
	...
	int _currentCollapseCount; // Current number of collapses performed
	...
	// Check if collapsing vFrom to vTo will flip any neighboring triangles
	bool checkCollapseFlip(Mesh& m, int vFrom, int vTo);
	// inside class PMesh, public section:
